"""
indicators.py — Technical indicators with advanced features.
"""
...
def is_volume_climax(df: pd.DataFrame, multiplier: float = 2.5) -> bool:
    """Volume climax: huge volume but price stalls."""
    if len(df) < 20:
        return False
    ...
    return last_vol > avg_vol * multiplier and last_close <= prev_close

# ========== Resistance Levels ==========
def find_resistance_levels(df: pd.DataFrame, lookback: int = 50, min_touches: int = 2,
                           tolerance_pct: float = 0.5) -> list[float]:
    ...
    while i < len(validated):
        ...
        while j < len(validated) and validated[j][0] <= level * 1.01:
            ...
            j += 1
        best = max(cluster, key=lambda l: np.sum((highs >= l*(1-tolerance_pct/100)) & (highs <= l*(1+tolerance_pct/100))))
...
def is_bearish_engulfing(df: pd.DataFrame) -> bool:
    ...
    if prev['close'] <= prev['open']:
        return False
    return curr['close'] < curr['open'] and curr['open'] >= prev['close'] and curr['close'] <= prev['open']
