...
- Риск на сделку: 5% от текущего депозита
...
RISK_PCT        = 0.05       # 5% риска на сделку (повышенный риск)
COMMISSION_PCT  = 0.0008     # 0.08% комиссия (taker × 2)
MAX_OPEN_TRADES = 5          # максимум одновременных позиций (повышенный риск)

# Плечо — более агрессивное распределение
LEVERAGE_OPTIONS = [5, 7, 10, 10, 12, 15]
