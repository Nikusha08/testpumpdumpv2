"""
strategy.py — Signal generation with multi-timeframe and weighted scoring.
"""
...
from indicators import (
    ...
    detect_liquidity_sweep, find_volume_profile_levels,
    detect_rsi_divergence, is_pin_bar, is_bearish_engulfing,
    is_volume_climax, is_weak_after_pump, get_macd, detect_macd_divergence
)

logger = logging.getLogger(__name__)

# ---------- CONFIGURATION (internal optimal values) ----------
class Config:
    # User-adjustable via env
    ...
    # Internal optimal parameters (do not expose)
...
def analyze_symbol(symbol: str, pump_pct: float) -> Optional[Signal]:
    ...
    rsi_ok = rsi_4h >= Config.RSI_OVERBOUGHT
    ...
    # RSI divergence
    rsi_div = detect_rsi_divergence(df_4h['high'], [calc_rsi(df_4h['close'].iloc[:i+1]) for i in range(len(df_4h))])
    macd, sig, _ = get_macd(df_4h)
    macd_div = detect_macd_divergence(df_4h, macd, sig)
    vol_climax = is_volume_climax(df_4h)

    # 8. Lower timeframe confirmations (1H, 15m)
    confirm_1h = False
    confirm_15m = False
    pin_1h = False
    if Config.USE_MULTI_TF:
        ...
        if df_1h is not None and len(df_1h) >= 20:
            ...
            if rsi_1h >= Config.RSI_OVERBOUGHT - 5:  # 70
                confirm_1h = True
    ...
    # Basic factors
    factors['pump'] = Config.WEIGHTS['pump'] * (min(pump_pct / Config.MIN_PUMP_PCT, 2.0))
    factors['volume_spike'] = Config.WEIGHTS['volume_spike'] if vol_spike else 0
    factors['rsi'] = Config.WEIGHTS['rsi'] * ((rsi_4h - 70) / 20) if rsi_4h > 70 else 0
    factors['resistance'] = Config.WEIGHTS['resistance'] if near_res else (Config.WEIGHTS['resistance'] * 0.5 if near_vp else 0)
    factors['funding'] = Config.WEIGHTS['funding'] * (funding / Config.MIN_FUNDING_RATE) if funding else 0
    factors['oi_divergence'] = Config.WEIGHTS['oi_divergence'] if oi_div else 0
    factors['liquidity_sweep'] = Config.WEIGHTS['liquidity_sweep'] if sweep else 0
    factors['pin_bar_4h'] = Config.WEIGHTS['pin_bar_4h'] if pin_4h else 0
    factors['pin_bar_1h'] = Config.WEIGHTS['pin_bar_1h'] if pin_1h else 0
    factors['rsi_divergence'] = Config.WEIGHTS['rsi_divergence'] if rsi_div else 0
    factors['macd_divergence'] = Config.WEIGHTS['macd_divergence'] if macd_div else 0
    factors['volume_climax'] = Config.WEIGHTS['volume_climax'] if vol_climax else 0
    factors['weak_after_pump'] = Config.WEIGHTS['weak_after_pump'] if weak else 0
    if Config.USE_MULTI_TF:
    ...
    # Simple confidence (max possible ~10, map to 0-100)
    max_possible = sum(Config.WEIGHTS.values()) * 1.5  # approximate
    ...
    # Additional guard: if using multi-TF, at least one lower timeframe confirmation
    if Config.USE_MULTI_TF and not (confirm_1h or confirm_15m) and score < Config.MIN_SCORE + 1:
        logger.debug(f"{symbol}: no lower TF confirmation")
        return None
